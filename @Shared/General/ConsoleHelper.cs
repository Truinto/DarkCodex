    ...
    public static class ConsoleHelper
    {
        ...
        public static bool DownloadFile(Uri uri, string filePath)
        {
            try
            {
                ...
                var request = client2.GetAsync(uri);
                //request.Wait();
                request.Result.Content.CopyTo(sw, null, default);
                sw.Close();
        ...
        /// </summary>
        public static int RunCommand(string command, string args, out string output)
        {
            ...
            p.Start();
            output = p.StandardOutput.ReadToEnd(); // TODO fix output always empty
            p.WaitForExit();
        ...
        public static Process RunCommandAsync(string command, string args = "", Action<string> onData = null, bool startNow = true)
        {
            ...
            p.OutputDataReceived += (sender, args) => onData?.Invoke(args.Data);
            p.ErrorDataReceived += (sender, args) => onData?.Invoke(args.Data);
            if (startNow)
            {
                p.Start();
        ...
        public static ManagementBaseObject GetDriveByVolume(string letter, List<ManagementBaseObject> drives)
        {
            ...
            drives ??= GetUSBDrives();
            var drive = drives.FirstOrDefault(f => letter.StartsWith((string)f["Caption"], StringComparison.OrdinalIgnoreCase));
            return drive;
        }
